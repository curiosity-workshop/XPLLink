...
namespace xpllink::serial
{
    ...
    LinuxSerialTransport::LinuxSerialTransport(
        std::string portName,
        std::uint32_t baudRate,
    ...
    bool LinuxSerialTransport::open()
    {
        if (isOpen())
        {
            return true;
    ...
    bool LinuxSerialTransport::configurePort()
    {
    ...
    void LinuxSerialTransport::applyControlMode()
    {
        int lines = TIOCM_DTR | TIOCM_RTS;

        if (controlMode_ == LinuxSerialControlMode::DtrRtsEnabled)
        {
    ...
    void LinuxSerialTransport::close()
    {
        if (!isOpen())
        {
            return;
    ...
    bool LinuxSerialTransport::isOpen() const
    {
        return descriptor_ >= 0;
    }

    std::size_t LinuxSerialTransport::read(
    ...
    std::size_t LinuxSerialTransport::write(
    ...
    const std::string&
        LinuxSerialTransport::portName() const
    {
        return portName_;
    }
}
